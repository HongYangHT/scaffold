...
<table cellpadding="0" cellspacing="0" align="center" bgcolor="#ffffff" v-set-style="style" v-set-parent-style="style" data-id="{{id}}">
	<tr v-for="n in row">
		<td v-for="i in col">
			<div data-id="{{id+(n+1)*(i+1)}}" v-on:click="showModal($event)" style="position:relative;width:100%;height:100%;" v-on:mouseenter="addClassActive($event)" v-on:mouseleave="removeClassActive($event)">
